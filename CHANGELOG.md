...
- Implemented OAuth2 Resource Server authentication for MCP server (MCP v2025-06-18 compliance)
  - Added `fastapi-oauth2-resource-server` dependency
  - Created `auth/oauth.py` with `OAuth2ResourceProtector` and token validation
  - Added required OAuth2 environment variables to `.env.example`
  - Enforced OAuth2 authentication on all API endpoints except health check
  - Created OAuth discovery document at `src/static/.well-known/mcp-oauth.json`
  - Mounted `.well-known` directory in FastAPI app to serve discovery document

### Changed

- Updated main API router to require OAuth2 authentication globally (except `/healthz`)

### Technical Details

- Replaced Personal Access Token (PAT) authentication with OAuth2 Bearer-JWT validation
- Prepared MCP server for OAuth2 introspection and protocol version negotiation
- Enhanced security and compliance with MCP 2025-06-18 specification
